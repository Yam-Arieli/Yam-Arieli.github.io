...
Two canonical breakpoints: **760px** for wide two-column layouts (a fixed-width rail next to
a fluid column) and **640px** for card-level flex rows. `.stack-phone` switches a
`display:flex` row to a column below 640px — add it alongside an existing inline flex row
rather than writing a one-off media query. Anything more specific (grid collapses, column
reflows) belongs in a `@media (max-width: 760px)` or `@media (max-width: 640px)` block inside
that page's own `<helmet><style>`, matching the pattern already in `Education.dc.html`.
Prefer grids built with `repeat(auto-fit,minmax(...,1fr))` over fixed-px tracks wherever the
content allows it — they reflow with no breakpoint needed at all.
